...
function add_countries() {
	if(document.id('shipto_virtuemart_state_id')) {
	new Request.JSON({
...
window.addEvent('domready',function() {
    ...
    //update_form(false);
    if(document.id('virtuemart_country_id').value=='') {
        document.id('virtuemart_country_id').value=<?php echo $params->get('country_id',0); ?>;
        jQuery("select.virtuemart_country_id").vm2front("list",{dest : "#virtuemart_state_id",ids : ""});
    }
    if(document.id('shipto_virtuemart_country_id').value=='') {
        document.id('shipto_virtuemart_country_id').value=<?php echo $params->get('country_id',0); ?>;
        add_countries();
    }
    update_form();
	...
	new Request.JSON({
		...
		'onSuccess':function(json,text) {
			if(json.error) {
				...
				document.getElements('tr.DBTaxRulesBill_tr').destroy();
				document.getElements('tr.taxRulesBill_tr').destroy();
				document.getElements('tr.DATaxRulesBill_tr').destroy();
				
				var row=0;
				for(var i=0;i<document.getElement('table.cart-summary').getElements('tr').length;i++) {
					if(document.getElement('table.cart-summary').getElements('tr')[i].getProperty('id')=='shipment_tr') {
						row=document.getElement('table.cart-summary').getElements('tr')[i-1];
						break;	
					}
				}
				
				
				for(var id in json.price.DBTaxRulesBill) {
					var tr=new Element('tr',{
						'class':'DBTaxRulesBill_tr'
					});
					tr.grab(new Element('td',{
						'colspan':4,
						'align':'right',
						'text':json.price.DBTaxRulesBill[id].title
					}));
					<?php if ( VmConfig::get('show_tax')) { ?>
						tr.grab(new Element('td'));
					<?php } ?>
					tr.grab(new Element('td',{
						'align':'right',
						'text':json.price.DBTaxRulesBill[id].value
					}));
					tr.grab(new Element('td',{
						'align':'right',
						'text':json.price.DBTaxRulesBill[id].value
				...
				for(var id in json.price.taxRulesBill) {
					var tr=new Element('tr',{
						'class':'taxRulesBill_tr'
					});
					tr.grab(new Element('td',{
						'colspan':4,
						'align':'right',
						'text':json.price.taxRulesBill[id].title
					}));
					<?php if ( VmConfig::get('show_tax')) { ?>
						tr.grab(new Element('td',{
							'align':'right',
							'text':json.price.taxRulesBill[id].value
						}));
					<?php } ?>
					tr.grab(new Element('td'));
					tr.grab(new Element('td',{
				...
				for(var id in json.price.DATaxRulesBill) {
					var tr=new Element('tr',{
						'class':'DATaxRulesBill_tr'
					});
					tr.grab(new Element('td',{
						'colspan':4,
						'align':'right',
						'text':json.price.DATaxRulesBill[id].title
					}));
					<?php if ( VmConfig::get('show_tax')) { ?>
						tr.grab(new Element('td'));
					<?php } ?>
					tr.grab(new Element('td',{
						'align':'right',
						'text':json.price.DATaxRulesBill[id].value
					}));
					tr.grab(new Element('td',{
						'align':'right',
						'text':json.price.DATaxRulesBill[id].value
